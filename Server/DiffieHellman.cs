...
namespace Server
{
    class DiffieHellman : IDisposable
    {
        ...
        {
            var key = CngKey.Import(keychung, CngKeyBlobFormat.EccPublicBlob); // tạo khóa bí mật từ khóa public (khóa public này là của client)
            keychungne = this.diffieHellman.DeriveKeyMaterial(key); // khóa bí mật tương đương với key chung, đây là common secret  
            this.aes.Key = keychungne; // add vào properties lúc này khóa secret (final) sẽ nằm ở Aes.key          
        }
